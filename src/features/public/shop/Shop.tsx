...
const Shop: React.FC = () => {
  ...
  const categories = [
    ...
    ...Array.from(new Set(products.map(p => p.category))).map(cat => ({
      ...
      count: products.filter(p => (typeof p.category === 'string' ? p.category : p.category?._id) === (typeof cat === 'string' ? cat : cat?._id)).length
    }))
  ];
  const priceRanges = [
  ...
  const filteredProducts = products.filter(product => {
    ...
    const matchesCategory = selectedCategory === 'all' || (typeof product.category === 'string' ? product.category : product.category?._id) === selectedCategory;
    ...
    const inStock = (product as any).inStock !== undefined ? (product as any).inStock : true;
    const matchesAvailability = selectedAvailability === 'all' || (selectedAvailability === 'in-stock' ? inStock : !inStock);
  ...
  const removeFromCart = (productId: string) => setCart(cart.filter(id => id !== productId));
  const isInCart = (productId: string) => cart.includes(productId);

  // Pour compatibilité avec ShopProductCard (image: string)
  const productsWithImage = sortedProducts.map(p => ({ ...p, image: p.images?.[0] || '' }));
        ...
        <div className="container mx-auto px-4">
          <AnimatedSection>
            <div className="text-center text-white">
              <h1 className="text-4xl md:text-5xl font-bold mb-6">Boutique Maker Skills</h1>
              <p className="text-xl max-w-3xl mx-auto opacity-90">Découvrez notre sélection de kits éducatifs, composants et outils pour vos projets</p>
            </div>
          </AnimatedSection>
        </div>
      </section>
