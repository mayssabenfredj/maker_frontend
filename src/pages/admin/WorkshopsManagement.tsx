...
import { Plus, Search, Calendar, Users, Clock, MapPin, Edit, Trash2, Eye } from 'lucide-react';
...
const WorkshopsManagement: React.FC = () => {
  ...
  const workshops = [
    {
      id: '1',
      title: 'Atelier Arduino Débutant',
    ...
    },
    {
      ...
      description: 'Apprenez la modélisation et l\'impression 3D',
    ...
    {
      id: '3',
      title: 'Atelier Robotique Mobile',
      description: 'Construction d\'un robot mobile autonome',
    ...
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className={`mt-2 ${
            ...
            Organisez et gérez vos ateliers pratiques
          </p>
        </div>
        <motion.button
          whileHover={{ scale: 1.05 }}
          whileTap={{ scale: 0.95 }}
          className="flex items-center space-x-2 px-6 py-3 bg-gradient-to-r from-purple-500 to-pink-600 text-white rounded-xl font-medium shadow-lg hover:shadow-xl transition-shadow"
        >
          <Plus className="h-5 w-5" />
          <span>Nouveau Workshop</span>
        </motion.button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {workshops.map((workshop, index) => (
          <AnimatedSection key={workshop.id} delay={index * 0.1}>
            <motion.div
              whileHover={{ y: -4 }}
              ...
              <div className="p-6">
                ...
                <div className="mb-4">
                  <div className="flex justify-between text-sm mb-1">
                    ...
                    <span className={`${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>
                      {Math.round((workshop.registeredParticipants / workshop.maxParticipants) * 100)}%
                    </span>
                  </div>
                  <div className={`w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2`}>
                    <div 
                      className="bg-gradient-to-r from-purple-500 to-pink-600 h-2 rounded-full transition-all duration-300"
                      style={{ width: `${(workshop.registeredParticipants / workshop.maxParticipants) * 100}%` }}
                    />
                  </div>
                </div>
